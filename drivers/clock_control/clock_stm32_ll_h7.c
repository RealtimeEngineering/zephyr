...
#elif CONFIG_CLOCK_STM32_MCO2_SRC_RC48
	#define MCO1_SOURCE		LL_RCC_MCO1SOURCE_RC48
#elif CONFIG_CLOCK_STM32_MCO2_SRC_PLL1Q
...
static int stm32_clock_control_init(const struct device *dev)
{
	...
	/* Initialize PLL 2 */
	r = get_vco_input_range(CONFIG_CLOCK_STM32_PLL2_M_DIVISOR, &vco_input_range);
	if (r < 0) {
		return r;
	}
...
#if defined(CONFIG_CLOCK_STM32_PLL2_N_FRACTION)
    LL_RCC_PLL2_SetFRACN(CONFIG_CLOCK_STM32_PLL2_N_FRACTION);
#else
	LL_RCC_PLL2FRACN_Disable();
#endif /* CLOCK_STM32_PLL2_N_FRACTION */
